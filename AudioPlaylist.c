...
void main(){
	...
	while(choice > 0){
  		...
  		printf("Insert Audio File	-	1\n");
  		printf("Delete Audio File	-	2\n");
  		printf("Play Audio File		-	3\n");
  		printf("Next Audio play		-	4\n");
  		printf("Prev Audio play		-	5\n");
  		printf("Exit			-	0\n");
  		printf("Enter Option: ");
		scanf("%d",&choice);
...
void Delete(char c[]){
	if(hptr==NULL)
		...
		int pos = Search(c,0);
		if(pos==-1)
			printf("Cannot delete as song not found.\n");
		else {
...
void Play(char b[]){
	int pos = Search(b,1);
	if(pos==-1)
		printf("Song to be played not found.\n");
	else {
		printf("\n\nCurrently Playing %s.\n\n",b);
	}
}
